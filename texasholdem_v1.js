...
class TexasHoldem {
    ...
    determineWinner() {
        ...
        let player1Hand = PokerSolver.solve([...this.players[0].hand, ...this.community_cards]);
        let player2Hand = PokerSolver.solve([...this.players[1].hand, ...this.community_cards]);

        let winners = PokerSolver.winners([player1Hand, player2Hand]);

        if (winners.length === 1) {
            let winningPlayer = (winners[0] === player1Hand) ? this.players[0] : this.players[1];
